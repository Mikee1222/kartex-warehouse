...
type ProductStockAdjustRowProps = {
  stock: number;
  onApply: (delta: number) => void;
  applying?: boolean;
};

export function ProductStockAdjustRow({
  stock,
  onApply,
  applying = false,
}: ProductStockAdjustRowProps) {
  const [draft, setDraft] = useState(1);
  const [sign, setSign] = useState<1 | -1>(1);
  const level: StockLevel = getStockLevel({ stock, min_stock: 0 });
  const stockClass =
    stock <= 0
      ? "text-[var(--red)]"
      : stock <= 5
  ...
  return (
    <div className="flex flex-col gap-3 py-4">
      <div className="flex items-center gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-[var(--text-muted)]">
            Προϊόν χωρίς χρωματικές παραλλαγές
          </p>
        </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          type="button"
          disabled={applying || (sign === -1 && stock <= 0)}
          onClick={() => {
            onApply(draft * sign);
            setDraft(1);
            setSign(1);
